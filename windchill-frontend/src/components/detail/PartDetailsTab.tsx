...
const PART_SECTIONS: SectionDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'BALDOWNSTREAM', label: 'Equivalence Network', format: 'equivNetwork' },
    ],
  },
  {
...
function parseEquivNetwork(value: string): { number: string; name: string; org: string; version: string; view: string }[] {
  if (!value) return []
  // Format: "NUMBER, NAME, ORG, VERSION (VIEW), NUMBER, NAME, ..." — comma-separated groups of 4
  const parts = value.split(',').map(s => s.trim())
  const entries: { number: string; name: string; org: string; version: string; view: string }[] = []
  let i = 0
  while (i + 3 < parts.length) {
    ...
    // Parse "00.1 (Manufacturing)" → version "00.1", view "Manufacturing"
