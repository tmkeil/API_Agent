    ...
    <div className="border-t border-slate-200 mt-3 pt-3 space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {data && (
            <>
              <span className="text-xs text-slate-500">{data.rows.length} rows · {data.rows.filter(r => r['PTp'] === 'L').length} parts · {data.rows.filter(r => r['PTp'] === 'D').length} docs · {data.rows.filter(r => r['PTp'] !== 'L' && r['PTp'] !== 'D').length} CAD</span>
              <button
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-2 rounded-md">
          {error}
          <button onClick={handleLoad} className="ml-3 underline">Retry</button>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg shadow-xl border border-slate-200 w-[600px] max-h-[80vh] flex flex-col">
              ...
              {sapResult && (
                <>
                  ...
                  <div className="text-xs text-slate-500 flex gap-4">
                    <span>Input: {sapResult.stats.totalInputRows} rows</span>
                    <span>Output: {sapResult.stats.totalOutputRows} SAP rows</span>
                    <span>Files: {sapResult.stats.filesCount}</span>
                  </div>
                  ...
                  {sapResult.validation.length > 0 && (
                    <div className="bg-amber-50 border border-amber-200 rounded-md px-4 py-3">
                      <div className="text-xs font-semibold text-amber-800 mb-2">
                        Validation — {sapResult.validation.length} issue{sapResult.validation.length !== 1 ? 's' : ''}
                      </div>
                  ...
                  {sapResult.files.length > 0 && (
                    <div>
                      <div className="text-xs font-semibold text-slate-700 mb-2">
                        CSV files ({sapResult.files.length})
                      </div>
            ...
            {sapResult && sapResult.files.length > 0 && (
              <div className="flex justify-end gap-2 px-5 py-3 border-t border-slate-200">
                ...
                >
                  Download all {sapResult.files.length} CSVs
                </button>
              </div>
            )}
          </div>
